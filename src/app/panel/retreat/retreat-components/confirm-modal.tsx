...
const ConfirmModal = <T,>({
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{waiting ? 'Confirme sua face!' : 'Esta tudo correto ?'}</DialogTitle>
          <DialogDescription>
          ...
          <>
            {products.some((p) => p.finality === 'collaborator') && (
              <ol className="my-5 divide-y ">
                {products
                  .filter((p) => p.finality === 'collaborator')
                  .map((prod) => (
                    <li key={prod.id}>
                      {' '}
                      {prod.quantity}x {prod?.product} {prod.size} {prod.service}
                    </li>
                  ))}
              </ol>
            )}
            ...
            {products.some((p) => p.finality === 'department') && (
              <ol className="my-5 divide-y ">
                {products
                  .filter((p) => p.finality === 'department')
                  .map((prod) => (
                    <li key={prod.id}>
                      {' '}
                      {prod.quantity}x {prod?.product} {prod.size} {prod.service}
        ...
        <DialogFooter>
          <div className="flex gap-2 justify-end">
            <Button
              disabled={waiting}
              className="mr-3 bg-btnGreen hover:bg-btnGreenHover"
              onClick={handleConfirm}
            >
              {waiting ? <Spin /> : 'Confirmar'}
            </Button>
            <DialogClose>
              <Button
                className="bg-btnRed hover:bg-btnRedHover close-modal-confirmation"
